...
vector<VarIdx> findInputChannelsInExpression(const arithmetic::Expression &e) {
	...
	for (const arithmetic::Operand &subExpr : e.exprIndex()) {
		// Iterate across all sub-expression leaves
		//TODO(steven.kneiser): erm, what if it's not an EXPR but just a var?? Ah, tends to be because when generated inside a function call, it tends to be nested in an expr
		const arithmetic::Operation &operation = *e.getExpr(subExpr.index);
		for (const arithmetic::Operand &operand : operation.operands) {
...
//TODO(steven.kneiser): could this be abstracted as a higher-level call above (e.g. this calls createVarDefBranch())?
// .,.because as it currently is scheduled last, I [pretty sure] can guarantee that there will always be a single assigner & assignee
// ...lol wait!
//TODO(steven.kneiser): ohhhh! Yeah this should only accept a transitionIdx! THERE ARE MULTIPLE RVARS being assigned, with an internal chan for each one. FINALYLL internalize channel creation
//NOTE(steven.kneiser): the guard should be isolated as a separate transition! That's a singular thing for each rvar in rexpr, but THIS
void graph::rewriteAssignmentAsChannel(TransitionIdx transitionIdx, VarIdx channelIdx) {
	...
	petri::iterator dummyTailIt = this->super::insert_after(transitionIt, chp::transition());
	...
	vector<VarIdx> sendVars = getVarsFromExpression(rexpr);
	for (VarIdx sendVar : sendVars) {

		if (sendVar >= this->vars.size()) { internal("", "ERROR: sendVar not in this->vars", __FILE__, __LINE__); continue; }
		if (varAssigned >= this->vars.size()) { internal("", "ERROR: varAssigned not in this->vars", __FILE__, __LINE__); continue; }
		string channelSuffix = ">>" + this->vars[varAssigned].name;
		VarIdx sendChannelIdx = this->getEnumeratedVar(sendVar, 0, channelSuffix + "!--");
		...
		arithmetic::Expression sendExpr = arithmetic::call(
				"send",
				{arithmetic::ident(arithmetic::Expression::varOf(sendChannelIdx)), rexpr}
				);
		...
		//if (not this->useDefChains.contains(varAssigned)) { internal("", "ERROR: varAssigned not in this->useDefChains", __FILE__, __LINE__); continue; }
		useDefChain sendChain;
		sendChain.varIdx = sendChannelIdx;
		if (sendChannelIdx >= this->vars.size()) { internal("", "ERROR: sendChannelIdx not in this->vars", __FILE__, __LINE__); continue; }
		sendChain.name = this->vars[sendChannelIdx].name;
		sendChain.isChannel = true;
		sendChain.isSend = true;
		sendChain.uses.insert(sendTransitionIdx);
		//sendChain.outs[sendTransitionIdx] = channelIdx;
		//recvChain.defs[sendTransitionIdx] = 1000 + channelIdx;  //TODO(steven.kneiser): just a readbility experiment
		this->useDefChains[sendChannelIdx] = sendChain;

		if (not this->useDefChains.contains(sendVar)) { internal("", "ERROR: sendVar not in this->useDefChains", __FILE__, __LINE__); continue; }
		this->useDefChains[sendVar].uses.erase(transitionIdx);
		this->useDefChains[sendVar].uses.insert(sendTransitionIdx);
		...
		arithmetic::Expression recvExpr = arithmetic::call(
				"recv",
				{arithmetic::ident(arithmetic::Expression::varOf(recvChannelIdx))}
				);
		...
		// Migrate lval Use-Def to new receiver
		useDefChain recvChain;
		recvChain.varIdx = recvChannelIdx;
		if (recvChannelIdx >= this->vars.size()) { internal("", "ERROR: recvChannelIdx not in this->vars", __FILE__, __LINE__); continue; }
		recvChain.name = this->vars[recvChannelIdx].name;
		recvChain.isChannel = true;
		recvChain.isSend = false;
		recvChain.uses.insert(recvTransitionIdx);
		//recvChain.ins[recvTransitionIdx] = channelIdx;  //TODO(steven.kneiser): just a readbility experiment
		//recvChain.defs[recvTransitionIdx].insert(1000 + channelIdx);  //TODO(steven.kneiser): just a readbility experiment
		this->useDefChains[recvChannelIdx] = recvChain;

		if (not this->useDefChains.contains(varAssigned)) { internal("", "ERROR: varAssigned not in this->useDefChains", __FILE__, __LINE__); continue; }
		this->useDefChains[varAssigned].defs.erase(transitionIdx);
		this->useDefChains[varAssigned].defs.insert(recvTransitionIdx);
	...
	///vector<VarIdx> sendVars = getVarsFromExpression(rexpr);  //sendExpr
	///// This is the send Expression, so be sure to prune the head target var

	///for (VarIdx sendVar : sendVars) {

	///	// Substitute new sendTransition for previous assignment
	///	//TODO(steven.kneiser): perf optimization: modify useDefChain in-place
	///	set<TransitionIdx> &sendVarUses = this->useDefChains[sendVar].uses;

	///	// Prune rewritten assignment if it existed (poerhaps we should only insert if it did??)
	///	//TODO(steven.kneiser): BIG OOPH! Not a VarIdx anymore! Gross type violation!
	///	if (sendVarUses.contains(transitionIdx)) {
	///		//sendVarUses.erase(transitionIdx);
	///		//sendVarUses.insert(sendTransitionIdx);
	///	}


	///	//TODO(steven.kneiser): cwe might have to check if this->useDefChains contains this, because I suspect some entries don't exist yet & need to be created
	///	//this->useDefChains[sendVar].name + varIdx + isChannel etc
	///	// ahh, at this point we're not TRYING to introduce new useDefs ...so in what cases does one side not yet exist? what drives the cretion of blan unamed/anonymous useDefs?

	///	//TODO(steven.kneiser): fix dis pwz
	///	//this->useDefChains[sendVar].uses = sendVarUses;
...
void graph::rewriteAssignmentAsCopyProcess(VarIdx varAssigned, TransitionIdx defTransitionIdx, unordered_map<VarIdx, set<ProjectionItem>> &projectionSets) {  //, const set<VarIdx> &uses
	clog << "rewriting assignment as Copy Process." << endl;

	if (not this->transitions.is_valid(defTransitionIdx)) { return; }
	const chp::transition &transition = this->transitions[defTransitionIdx];
	...
	//vector<petri::iterator> forkOutPlaceIts = this->next(forkTransitionIt);
	//if (forkOutPlaceIts.empty()) { internal("", "forkTransition has no out-places", __FILE__, __LINE__); return; }
	//petri::iterator forkOutPlaceIt = forkOutPlaceIts[0];

	//vector<petri::iterator> forkOutTransitionIts = this->next(forkOutPlaceIt);
	//if (forkOutTransitionIts.empty()) { internal("", "forkOutPlace has no out-transitions", __FILE__, __LINE__); return; }
	//petri::iterator dummyTailIt = forkOutTransitionIts[0];



	//petri::iterator dummyHeadIt = this->super::insert_after(forkTransitionIt, chp::transition());
	...
	//this->rewriteAssignmentAsChannel(forkTransitionIt.index, forkChannelIdx);  //TODO(steven.kneiser): silenced since we now wait until after ALL Copy Process forks & branches are done to channel-substitute them at the very end


	// Remove the original assignment [no longer needed w/ fork creation batched beforehand
...
void graph::rewriteEachSingleUseVarAsDirectChannel(
	...
	//TODO(steven.kneiser): attempted rewrite for Use-Defs >> invDepSets (when did recv-channels sneak into here??)
	//////TODO(steven.kneiser): s/varIdx/usedVarIdx
	////for (auto &[varIdx, useChain] : this->useDefChains) {
	////	if (useChain.uses.size() != 1) { continue; }

	////	TransitionIdx useTransitionIdx = *useChain.uses.begin();
	////	if (not this->transitions.is_valid(useTransitionIdx)) { internal("", "ERROR: useTransitionIdx isn't valid", __FILE__, __LINE__); continue; }
	////	const chp::transition &useTransition = this->transitions[useTransitionIdx];

	////	VarIdx user = std::numeric_limits<VarIdx>::max();
	////	const arithmetic::Choice &choice = useTransition.action;
	////	for (const arithmetic::Parallel &term : choice.terms) {
	////		for (const arithmetic::Action &action : term.actions) {
	////			if ((not action.lvalue.isUndef()) and action.lvalue.top.type == arithmetic::Operand::Type::VAR) {
	////				user = action.lvalue.top.index;
	////				break;
	////			}
	////		}  //TODO(steven.kneiser): make robust against multi-lvals. This will unintenionally return the last lvar.
	////	}
	////	if (user == std::numeric_limits<VarIdx>::max()) { continue; }

	////	if (user >= this->vars.size() or varIdx >= this->vars.size()) { internal("", "ERROR: user (or) varIdx not in vars", __FILE__, __LINE__); continue; }
	////	clog << endl << " ===> ==> => > " << this->vars[user].name << ":= ... " << this->vars[varIdx].name << endl;

	////	//TODO(steven.kneiser): redundant w/ `chain` in loop scope?
	////	//if (not this->useDefChains.contains(user)) { internal("", "ERROR: user not in this->useDefChains", __FILE__, __LINE__); continue; }
	////	//useDefChain &userChain = this->useDefChains[user];
	////	//TODO(steven.kneiser): reimpl from ProjSets to UseDefs

	////	//if (userChain.defs.empty()) { continue; }

	////	VarIdx varUsageIdx = this->getEnumeratedVar(user, 0, "~lone--");
	////	this->remapVarInEachTransition(user, varUsageIdx);
	////	//TODO(steven.kneiser): there exists a powerful parallel here to the need to retrieve the transition FROM the var's target/def-er/user that we do for Copy Processes
	////	// Ideally, we should just be doing a singular surgical remapping in "this" one transition, not sloppily innefficiently search ALL transitions

	////	// Rewrite "x = ..." as "CHAN.send(x)" & "x_usage_n = CHAN.recv()" in parallel
	////	//TransitionIdx userDefTransitionIdx = *userChain.defs.begin();
	////	//petri::iterator userDefTransitionIt(petri::transition::type, userDefTransitionIdx);
	////	VarIdx channelIdx = this->getEnumeratedVar(user, 0, "~LONE_CHAN--");
	////	this->rewriteAssignmentAsChannel(useTransitionIdx, channelIdx);

	////	projectionSets[varIdx].insert(ProjectionItem(channelIdx, true, true));
	////	projectionSets[user].insert(ProjectionItem(channelIdx, true, true));
	////	//TODO(steven.kneiser): verify nothing needs to be REMOVED from Projection Sets after rewrite
	////}
...
void graph::rewriteEachMultiUseVarAsCopyProcess(
	...
	for (const auto &[dependency, users] : invertedDependencySets) {
		...
		//if (chain.defs.empty()) { cerr << "ERROR: no definition found in useDefChain for var `" << chain.name << "`. Skipping all branch additions." << endl; continue; }
		...
		this->rewriteAssignmentAsCopyProcess(dependency, forkTransitionIdx, projectionSets);  //users); forkChannelIdx);
	}
	...
	for (const controlFlowBlock &block : this->controlFlowGraph) {
		...
		for (VarIdx guardVarIdx : allOutGuardVars) {
			...
			for (auto &[defTransitionIdx, defVarIdx] : outGens) {
				...
				//TODO(steven.kneiser): Create dependsOn + requiredFor relationship
				...
				for (BlockIdx outBlockIdx : block.outs) {
					...
					for (auto &[branchVarIdx, localDefVarIdx] : branchVarIdxs) {
						...
						//for (VarIdx assignedVar : assignedVars) {
							...
							//	useDefChain &chain = this->useDefChains[assignedVar];
...
TransitionIdx graph::createVarDefBranch(VarIdx sourceVarIdx, VarIdx branchVarIdx) { //, TransitionIdx targetTransitionIdx
	...
	//TODO(steven.kneiser): these should ensure they grab the dummy head
...
void graph::createCopyProcessForksForMultiUseVars(unordered_map<VarIdx, set<ProjectionItem>> &projectionSets) {
	clog << "creating all Copy Processes in batch." << endl;

	for (auto &[varIdx, chain] : this->useDefChains) {
		if (chain.uses.size() < 2) { continue; }
		...
		chp::transition forkTransition(Expression::vdd(), arithmetic::Choice({{forkAssignment}}));
		petri::iterator defTransitionIt(petri::transition::type, defTransitionIdx);
		petri::iterator forkTransitionIt = this->super::insert_after(defTransitionIt, forkTransition);
		TransitionIdx forkTransitionIdx = forkTransitionIt.index;
		...
		petri::iterator dummyTailIt = this->super::insert_after(forkTransitionIt, chp::transition());
		...
		chain.copyProcess = forkTransitionIdx;  //TODO(steven.kneiser): egregious type violation, should be UseDefIdx
																								 //  ...where we look it up via "chain.copyProcess.def/s[0]",
		...
		for (VarIdx usedVarIdx : usedVarLookup) {
			...
			usedVarChain.uses.insert(forkTransitionIdx);
			usedVarChain.uses.erase(defTransitionIdx);
		}

		// Update Projection Sets
...
void graph::rewriteAssignmentsAsChannels(
	...
	this->createCopyProcessForksForMultiUseVars(projectionSets);
